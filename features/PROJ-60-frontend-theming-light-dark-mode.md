...
- **Post-deploy bugfix (2026-07-20):** user reported unreadable chat error messages in Light mode (`text-red-300` on `bg-red-900/30`/`border-red-700/50` — a dark-mode-only combo). Root cause: `Chat/renderers/ErrorMessage.tsx` and `Chat/renderers/ToolCallMessage.tsx` use literal `red-*` classes, which the original gray-only migration sweep didn't catch (only `bg-gray-*`/`text-gray-*` were targeted). Fixed both to use the existing semantic `destructive`/`destructive-foreground` tokens (already defined in `globals.css` for both themes, already used by the app's shadcn `Alert` destructive variant elsewhere) instead of literal colors: `border-destructive/50 dark:border-destructive`, `bg-destructive/10`, `text-destructive`. Checked remaining `red-*` usage in `Chat/InputArea.tsx` (recording-indicator button) — solid `bg-red-600`/white-text/`ring-red-400`, not a translucent-on-translucent combo, already contrast-safe in both themes, left untouched. Rebuild confirmed clean.
