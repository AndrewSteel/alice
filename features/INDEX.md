...
| PROJ-95 | 2     | BankTransaction-Thumbnails — Text-Rendering-Modus analog PROJ-93 (kein `filePath` im Schema, da aus BankStatement generiert); Thumbnail-Text aus `purpose`/`counterparty`/`amount` der Transaktion + Kontext (z.B. Bankname) aus dem per `parentStatementId` referenzierten BankStatement (zusätzlicher Weaviate-Lookup nötig); bei PROJ-55-Refine 2026-08-28 entdeckt (`Code: Query Weaviate` schlägt für BankTransaction mit GraphQL-Fehler fehl, da `filePath`/`fileHash` im Schema fehlen) | Roadmap | —    | 2026-08-28 |
| PROJ-96 | 3.1   | Image-Description-Backfill Auto-Fortsetzung + Zeitlimit — `alice-dms-image-description-backfill` holt pro Aufruf nur max. 50 Bilder ohne Auto-Continue (bei 24310 Bildern mit `extraction_failed=true` wären 487 manuelle Webhook-Aufrufe nötig) und hat kein Zeitlimit (Ollama-Vision-Timeout 5min/Bild, sequenziell); analog PROJ-94 (Zeitlimit) + PROJ-92 (Backfill-Confirm-Gate-Muster); bei PROJ-55-Refine 2026-08-28 entdeckt, Workflow noch nie ausgeführt | Approved | [Spec](PROJ-96-image-description-backfill-auto-continue-time-limit.md) | 2026-08-28 |

<!-- Add features above this line -->

## Next Available ID: PROJ-97