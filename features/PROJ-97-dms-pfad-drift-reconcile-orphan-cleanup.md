...
Ein neuer manueller Batch-Workflow **`alice-dms-reconcile`**, der den Weaviate-Bestand gegen die von Scanner/Path-Worker gepflegte Redis-Wahrheit (`alice:dms:path_to_hash` / `alice:dms:hash_to_paths:<hash>`) abgleicht:
...
6. **`confirm`-Gate.** Ohne `confirm=true` reiner Dry-Run: identifiziert + zählt + loggt alle Waisen und reparierbaren Objekte, schreibt nichts. Mit `confirm=true` werden Reparatur-PATCH und Löschungen ausgeführt.
7. **Redis-Lock + optionales Zeitlimit.** `alice:dms:processor:lock:run` (fail-closed, sauberer Skip bei belegtem Lock). Optionaler `time_limit_seconds`-Parameter (per-Objekt-Zeitcheck, sauberer Stopp mit vollständigem Response, kein Auto-Continue).
...
- [ ] `alice-dms-reconcile` wird per `POST /webhook/alice-dms-reconcile` ausgelöst.
- [ ] Ohne `confirm=true` im Body: Dry-Run — der Lauf identifiziert alle Kandidaten, zählt sie nach Kategorie, loggt jeden einzelnen per winston (UUID, Collection, letzter bekannter `filePath`, geplante Aktion), führt **keine** Weaviate-Schreib-/Löschoperation und **kein** MQTT-Publish aus.
- [ ] Mit `confirm=true`: Reparatur-PATCH, Löschungen und `alice/dms/done`-Publishes werden ausgeführt.
- [ ] `alice-dms-reconcile` nimmt vor Verarbeitungsbeginn den Redis-Lock `alice:dms:processor:lock:run` (NX, TTL als Fallback). Ist der Lock belegt, beendet sich der Workflow sauber mit `{ status: 'skipped', stopped_reason: 'locked', message: 'Lauf bereits aktiv' }`, HTTP 200, ohne zu warten. Der Lock wird nach Abschluss (Erfolg, Zeitlimit-Stopp, Fehler) zuverlässig freigegeben.
- [ ] Optionaler `time_limit_seconds`-Parameter im Body: gesetzt → nach jedem verarbeiteten Objekt Zeitcheck, bei Ablauf sauberer Stopp mit vollständigem Response; nicht gesetzt → alle Objekte über alle geprüften Collections werden verarbeitet. Kein Selbstaufruf/Auto-Continue.
...
**Response:**
- [ ] Der Webhook-Response enthält ausschließlich aggregierte Zähler: `{ status, mode: 'dry-run' | 'confirm', stopped_reason: 'completed' | 'time_limit' | 'locked', checked, path_repaired, deleted_orphan, deleted_cascade_banktx, deleted_orphan_banktx, thumbnail_files_removed, done_events_published, skipped_ok, remaining }`.
- [ ] Jede einzelne Aktion (Reparatur, Löschung, Cascade) wird per winston geloggt mit UUID, Collection, letztem bekannten Pfad und Aktion.
...
- `time_limit_seconds` per Node-Namen-Zugriff auf den Webhook-Node lesen (nicht `$input.first().json.body`) — der vorgeschaltete Lock-Node liefert ein Item ohne `body` (PROJ-92-Bug).
